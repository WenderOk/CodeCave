...
--        ('Akira', 'Sato', 'Japan', 'akiro.s@yagu-yagu.jp', 476),
...
-- select avg(rating) as avg_usa_rating from Directors where country = "USA";
-- select count(rating) as num_of_rated_usa_directors from Directors where country = "USA" and rating > 50;
-- select count(distinct country) as num_of_countries from Directors;
-- select min(rating) as min_rating, max(rating) as max_rating from Directorswhere country="USA";
select group_concat(distinct country order by country separator ", ") as countries from Directors;
